...
export function CreateAssignmentForm({ courseOptions }: CreateAssignmentFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/office/assignments", {
        method: "POST",
        body,
      });
      ...
      if (res.ok && data.success) {
        router.push(`/office/assignments/${data.assignmentId}`);
        router.refresh();
      } else {
        setErrors({ form: data.error || "Failed to create assignment" });
      }
    } catch (error) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6" noValidate>
      ...
      <div className="flex items-center gap-3">
        ...
        <Label htmlFor="isPublished" className="cursor-pointer text-sm text-slate-700">
          Publish immediately (students will see this assignment)
        </Label>
      </div>

      <div className="flex justify-end gap-3 pt-4 border-t border-slate-200">
        <Button type="button" variant="outline" asChild>
          <Link href="/office/assignments">Cancel</Link>
        </Button>
